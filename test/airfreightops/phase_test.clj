(ns airfreightops.phase-test
  "Unit tests of `airfreightops.phase` rollout logic."
...
(deftest phase-1-shipment-record-only
  (testing "phase 1 allows only shipment-record logging, requires approval"
    ...
    (let [{:keys [disposition]} (phase/gate 1 {:op :schedule-ground-operation} :commit)]
      (is (= :hold disposition)))))
...
(deftest phase-3-auto-commits-clean-ops
  (testing "phase 3 auto-commits clean, high-conf non-safety ops"
    ...
    (let [{:keys [disposition]} (phase/gate 3 {:op :schedule-ground-operation} :commit)]
      (is (= :commit disposition)))
...
(deftest no-flight-ops-op-exists-in-any-phase-writes
  (testing "structural invariant: no phase's :writes set contains anything but the closed four-op allowlist -- there is no flight-operations op to gate in the first place"
    (doseq [[ph {:keys [writes]}] phase/phases]
      (is (every? #{:log-shipment-record :schedule-ground-operation
                    :coordinate-maintenance-order :flag-safety-concern} writes)
          (str "phase " ph " :writes must only ever contain the closed ground-logistics allowlist")))))
