(ns airfreightops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (AirFreightAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the AirCargoGroundOpsGovernor
  ...
  human ground-logistics coordinator (or, for a safety concern, the
  operating crew/cargo-safety officer). The approver resumes with
  `{:approval {:status :approved}}` (or :rejected).

  This actor coordinates GROUND LOGISTICS SCHEDULING ONLY -- it never
  touches flight operations, piloting, or air-traffic-control functions
  in any way, and never authorizes a flight to depart. See
  `airfreightops.governor` for the HARD scope invariants."
...
(defn build
  ...
  `airfreightops.store/Store`).
  opts:
    :advisor      -- an `airfreightops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; AirFreightAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; AirCargoGroundOpsGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :facility-id (:facility-id request)
                        :reason (or reason
      ...
      ;; Approval handoff -- paused by interrupt-before; a human ground-
      ;; logistics coordinator (or operating crew/cargo-safety officer
      ;; for a safety concern) resumes with :approval. Then route
      ;; commit/hold.
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :facility-id (:facility-id request) :by (:by approval)}]}
            {:disposition :hold
