(ns airfreightops.phase
  "Phase 0->3 staged rollout for the ISIC-5120 'Freight air transport'
  (cargo airline and air freight forwarding) GROUND LOGISTICS SCHEDULING
  coordination actor.

    Phase 0  read-only              -- no writes, still governor-gated.
    Phase 1  assisted-logging       -- shipment-record logging allowed,
                                       every write needs human approval.
    Phase 2  assisted-coordination  -- adds ground-operation scheduling
                                       and maintenance-order proposals,
                                       still approval-gated.
    Phase 3  supervised auto        -- governor-clean, high-confidence
                                       `:log-shipment-record`/
                                       `:schedule-ground-operation`/
  ...
  rollout milestone still to come. Flagging a weight-and-balance/
  dangerous-goods/airworthiness concern always needs a human (operating
  crew/cargo-safety officer) to actually look at it, and must escalate
  IMMEDIATELY -- safety-critical, never merely a business-process
  convenience. `airfreightops.governor`'s own `always-escalate-ops`
  enforces the same invariant independently -- two layers, not one,
  agree on this."
  (:require [airfreightops.governor :as governor]))
...
(def phases
  ...
  {0 {:label "read-only"              :writes #{}                                                              :auto #{}}
   ...
   2 {:label "assisted-coordination"  :writes #{:log-shipment-record :schedule-ground-operation
                                                 :coordinate-maintenance-order}                                :auto #{}}
   3 {:label "supervised-auto"        :writes write-ops
      :auto #{:log-shipment-record :schedule-ground-operation :coordinate-maintenance-order}}})
...
(defn verdict->disposition
  "Map an AirCargoGroundOpsGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
